...
// VG12 - 12bit - 12.02 - Complete and tested both low and high sample
// VGDP - digipot 12.02 - Programmable bit-banging complete and tested
...
uint8_t DigiZeroLevel = 75; // VGDP, 75 - 407 Ohms in digipot
uint8_t counter = 0x80; // VGDP, counter for allowing configuration, BINARY - 10000000, Decimal - 128
...
uint8_t RADIO_PACKET_LENGTH = SMALL_PACKET_LENGTH;  //VG12 // OG // Reserve enough memory for the largest packet size possible
// uint8_t RADIO_PACKET_LENGTH = LARGE_PACKET_LENGTH;  //VG12
...
static uint32_t LARGE_PACKET_PAYLOAD_12Bit = (LARGE_PACKET_LENGTH - RADIO_DATA_OFFSET - STIM_STATUS_LENGTH) * 2 / 3; //VG12 4/5
...
// static uint32_t SMALL_PACKET_PAYLOAD_12Bit = (SMALL_PACKET_LENGTH - RADIO_DATA_OFFSET - STIM_STATUS_LENGTH) * 2 / 3; //VG12 3/5
static uint32_t SMALL_PACKET_PAYLOAD_12Bit = (SMALL_PACKET_LENGTH - RADIO_DATA_OFFSET - STIM_STATUS_LENGTH) * 3 / 5; //VG12 11/16
...
const uint8_t Mask3[2]={0xff, 0x0f}; //Mask3 used for packet stuffing 12-bit samples, VG12
const uint8_t Mask4[2]={0xf0, 0xff}; //Mask4 used for packet stuffing 12-bit sample, VG12
...
uint8_t tx_data[TX_RX_MSG_LENGTH]; //!< SPI TX buffer // TX_RX is 2 bytes
...
*/
  ...
  // tx_data[0] = (DacZeroLevel>>8); //old DAC
...
int GPIO_Init(void)
{
  ...
  //Set the gpio BATT_SW_CTRL output pin to be an output
  NRF_GPIO->PIN_CNF[BATT_SW_CTRL] = GPIO_PIN_CNF_DIR_Output;
  NRF_GPIO->DIRSET |= 1 << BATT_SW_CTRL;
  NRF_GPIO->OUT &= ~(1<<BATT_SW_CTRL);

  // //Set the gpio BIONODE_GPIO_1 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_1] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_1;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_1);

  // //Set the gpio BIONODE_GPIO_2 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_2] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_2;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_2);

  // //Set the gpio BIONODE_GPIO_3 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_3] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_3;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_3);

  // //Set the gpio BIONODE_GPIO_4 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_4] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_4;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_4);

  // //Set the gpio BIONODE_GPIO_5 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_5] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_5;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_5);

  // //Set the gpio BIONODE_GPIO_6 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_6] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_6;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_6);

  // //Set the gpio BIONODE_GPIO_7 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_7] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_7;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_7);

  // //Set the gpio BIONODE_GPIO_8 output pin to be an output
  // NRF_GPIO->PIN_CNF[BIONODE_GPIO_8] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->DIRSET |= 1 << BIONODE_GPIO_8;
  // NRF_GPIO->OUT &= ~(1<<BIONODE_GPIO_8);

  //// Set the gpio test output pin to be an output
  NRF_GPIO->PIN_CNF[TEST1_GPIO_OUT] = GPIO_PIN_CNF_DIR_Output;
  NRF_GPIO->DIRSET |= 1 << TEST1_GPIO_OUT; 
  NRF_GPIO->OUT &= ~(1<<TEST1_GPIO_OUT); //set low

  // //Set the gpio test2 output pin to be an output
  NRF_GPIO->PIN_CNF[TEST2_GPIO_OUT] = GPIO_PIN_CNF_DIR_Output;
  // NRF_GPIO->PIN_CNF[TEST2_GPIO_OUT] = GPIO_PIN_CNF_PULL_Pullup,   
  NRF_GPIO->DIRSET |= 1 << TEST2_GPIO_OUT; 
  NRF_GPIO->OUT &= ~(1<<TEST2_GPIO_OUT); //set low
 
  // //Set the gpio test3 output pin to be an output
  NRF_GPIO->PIN_CNF[TEST3_GPIO_OUT] = GPIO_PIN_CNF_DIR_Output;
  NRF_GPIO->DIRSET |= 1 << TEST3_GPIO_OUT; 
  NRF_GPIO->OUTSET = (1<<TEST3_GPIO_OUT);//set high
  // NRF_GPIO->OUT &= ~(1<<TEST3_GPIO_OUT);

  return 0;
}

//// VG-DP Add digipopt stuff here
  ...
  else
  {
    ...
    if (NRF_RADIO->CRCSTATUS == RADIO_CRCSTATUS_CRCSTATUS_CRCOk)
    {
      ...
      if (RadioRxData[1] != 0) //make sure the PID is not zero. This would indicate a non-updating handshake
      {
        ...
        // /////// VGDP digipot - Resolution = 1200 Ohms / 256 taps = 4.69 Ohms with 20% tolerance    
        /////// Working - Not programmable
        // //// set CS low
        // NRF_GPIO->OUTCLR = (1<<TEST3_GPIO_OUT);  
        
        // //delay before next clock cycle
        // for (int i=0;i<500;i++){}  

        // // /// CLK
        // // /// 500us (2000 counter) high, 500us low
        // //clock high, clock low, repeat twice
        // for (int i=0;i<2;i++){ //1000 - 250us high 
        //   for (int i=0;i<2000;i++){ //1000 - 250us high
        //     NRF_GPIO->OUTSET = (1<<TEST1_GPIO_OUT); 
        //   }
        //   //clock low
        //   for (int i=0;i<2000;i++){
        //     NRF_GPIO->OUTCLR = (1<<TEST1_GPIO_OUT); 
        //   }
        // }
        // // set first data bit as 1
        // NRF_GPIO->OUTSET = (1<<TEST2_GPIO_OUT); 
        
        // //delay before next clock cycle to latch data bit 1
        // for (int i=0;i<500;i++){} 

        // // begin 3rd clock cycle high
        // for (int i=0;i<2000;i++){ //1000 - 250us high
        //     NRF_GPIO->OUTSET = (1<<TEST1_GPIO_OUT); 
        // }
        // //set 3rd clock clock low
        // for (int i=0;i<2000;i++){ 
        // NRF_GPIO->OUTCLR = (1<<TEST1_GPIO_OUT); 
        // }

        // //set 4th clock cycle high and low
        // for (int i=0;i<2000;i++){ //1000 - 250us high
        //     NRF_GPIO->OUTSET = (1<<TEST1_GPIO_OUT); 
        // }
        // //set 4th clock clock low
        // for (int i=0;i<2000;i++){ 
        // NRF_GPIO->OUTCLR = (1<<TEST1_GPIO_OUT); 
        // }

        // // set second data bit and all subsequent data bits as 0
        // NRF_GPIO->OUTCLR = (1<<TEST2_GPIO_OUT); 

        // //delay before next clock cycle to latch data bit 1
        // for (int i=0;i<500;i++){} 
        
        // //clock high, clock low for 7 remaining cycles
        // for (int i=0;i<6;i++){ //1000 - 250us high 
        //   for (int i=0;i<2000;i++){ //1000 - 250us high
        //     NRF_GPIO->OUTSET = (1<<TEST1_GPIO_OUT); 
        //   }
        //   //clock low
        ...
        // }
        // //// set CS high
        // NRF_GPIO->OUTSET = (1<<TEST3_GPIO_OUT);
        // //// Working - not programmable

        //If using SPI
        // tx_data[0] = (DigiZeroLevel>>2); //VG NEW DAC
        // tx_data[1] = (6<<DigiZeroLevel)&0xFF; //VG NEW DAC 
        // spi_master_tx_rx(spi_base_address, TX_RX_MSG_LENGTH, (const uint8_t *)tx_data, rx_data);
        ...
        //set CS low to begin transmission
        NRF_GPIO->OUTCLR = (1<<TEST3_GPIO_OUT); 
        //set Din low to begin transmission
        NRF_GPIO->OUTCLR = (1<<TEST2_GPIO_OUT); 
        
        // //delay before next clock cycle
        for (int i=0;i<500;i++){}

        //set clock high, clock low, twice for first two bits
        for (int i=0;i<2;i++){ //1000 - 250us high 
            for (int i=0;i<2000;i++){ //1000 - 250us high
            NRF_GPIO->OUTSET = (1<<TEST1_GPIO_OUT); 
            } 
        ...
        //Configurable stuff begins now
        uint8_t DigiZeroLevel = RadioRxData[POS_DIGIPOT];
        uint8_t counter = 0x80; // VGDP, counter for allowing configuration, BINARY - 10000000, Decimal - 128
        
        for (int i=0;i<8;i++){
            if ((DigiZeroLevel&counter) == counter) {
                for (int i=0;i<1000;i++){ //1000 - 250us high
                  NRF_GPIO->OUTSET = (1<<TEST2_GPIO_OUT); 
                } 
                // NRF_GPIO->OUTSET = (1<<TEST2_GPIO_OUT);  //set Din high
                 
            } else {
                for (int i=0;i<1000;i++){ //1000 - 250us high
                  NRF_GPIO->OUTCLR = (1<<TEST2_GPIO_OUT); 
                } 
                // NRF_GPIO->OUTCLR = (1<<TEST2_GPIO_OUT);  //set Din low
            }
            ////delay before next clock cycle to latch data bit 1
            // for (int i=0;i<500;i++){} 
            
            //set clock high after figuring out whether Din is high or low
            for (int i=0;i<2000;i++){ //1000 - 250us high
            NRF_GPIO->OUTSET = (1<<TEST1_GPIO_OUT); 
            }
            //set clock low
            ...
            //set DIN low now in case it was high before
            NRF_GPIO->OUTCLR = (1<<TEST2_GPIO_OUT);  //set Din low
            counter = counter>>1; //bitshift counter by 1
        }
        //set CS high to end transmission
        NRF_GPIO->OUTSET = (1<<TEST3_GPIO_OUT); 
        //////// VGDP WORKING - Programmable
        ...
        if ((NRF_TIMER1->CC[0] != RadioRxData[POS_ADC_TIMER1_CC0_REG]) || (NRF_TIMER1->PRESCALER != RadioRxData[POS_ADC_TIMER1_PRESCALE_REG]))
        {
          ...
          PACKET_PAYLOAD_LENGTH = RADIO_PACKET_LENGTH - RADIO_DATA_OFFSET - STIM_STATUS_LENGTH; //VG, 180 bytes (large) or 40 bytes (small)
          if (RadioRxData[POS_ADC_RESOLUTION] == 10){
            PACKET_PAYLOAD_LENGTH -= ((RADIO_PACKET_LENGTH - RADIO_DATA_OFFSET - 5) / 5); //VG 144 bytes (large) or 32 bytes(small)
          }
          //VG12 12-bit
          if (RadioRxData[POS_ADC_RESOLUTION] == 12){ 
            if (RADIO_PACKET_LENGTH == SMALL_PACKET_LENGTH){
              PACKET_PAYLOAD_LENGTH = 24; //VG-11/18 //26 //Add 8 trailing zeros because should actually be 26.6 bytes, 26
            }
            else {
              PACKET_PAYLOAD_LENGTH -= ((RADIO_PACKET_LENGTH - RADIO_DATA_OFFSET - 5) / 3); //VG12 120 bytes payload (large)
            }
          }
        ...
        if ( (ADC_Active_Ch != ((RadioRxData[POS_ACTIVE_CHANNELS_H] <<8) | RadioRxData[POS_ACTIVE_CHANNELS_L])) || (ADC_Resolution != RadioRxData[POS_ADC_RESOLUTION]))
        {
          ...
          PACKET_PAYLOAD_LENGTH = RADIO_PACKET_LENGTH - RADIO_DATA_OFFSET - STIM_STATUS_LENGTH ;
           ...
           //VG 10 bit stuff verified - why is this done here again when its done in line 757? //VG12
          ...
          if (ADC_Resolution == 12){
            if (RADIO_PACKET_LENGTH == SMALL_PACKET_LENGTH){
              PACKET_PAYLOAD_LENGTH -= (((RADIO_PACKET_LENGTH - RADIO_DATA_OFFSET - 5) * 4) / 10); //VG12 OG 
            }
            else { //large packet
            PACKET_PAYLOAD_LENGTH -= ((RADIO_PACKET_LENGTH - RADIO_DATA_OFFSET - 5) / 3); //VG12 11-18
            }
          } //VG12
...
void SAADC_IRQHandler(void)
{
  ...
  static uint8_t Cnt_II = 0; //Counter for 12-bit packet stuffing
  static uint8_t Cnt_III = 0; //Counter for 12-bit packet stuffing
  ...
  // tempVal = 0x3ff & *((uint32_t *)NRF_SAADC->RESULT.PTR);// VG12 OG
  tempVal = 0xfff & *((uint32_t *)NRF_SAADC->RESULT.PTR); //VG12
  ...
  if ((ADC_Counter & 0x3) == 0)
    NRF_SAADC->CH[0].PSELP = SAADC_Active_PSELP_Registers[1] << SAADC_CH_PSELP_PSELP_Pos;//point to channel 2 next
  else if ((ADC_Counter & 0x3) == 1) //bitwise AND, modulus function or hard coded to have all the counts. 0-1-2-3. go back to zero, then track youre going back. 
    NRF_SAADC->CH[0].PSELP = SAADC_Active_PSELP_Registers[2] << SAADC_CH_PSELP_PSELP_Pos;// point to channel 3 next
    ...
    NRF_SAADC->CH[0].PSELP = SAADC_Active_PSELP_Registers[0] << SAADC_CH_PSELP_PSELP_Pos; //back to channel 1
  ...
  ////////VG12
  else if (ADC_Resolution == 12)
  {
  if ((ADC_Counter & 0x01) == 0x00) { //The if statement above looks at LSB to see if its a multiple of 2. 
  //// After two 12-bit samples, I need to packet stuff into three 8bit packets
  //// zero out three 8bit packets
    Radio_Buffer[Cnt_III+Cnt_II*3+RADIO_DATA_OFFSET] = 0;
    Radio_Buffer[Cnt_III+Cnt_II*3+RADIO_DATA_OFFSET +1] = 0;
    Radio_Buffer[Cnt_III+Cnt_II*3+RADIO_DATA_OFFSET +2] = 0;
  }
  //packing one ADC sample at a time, properly
  Radio_Buffer[Cnt_III+Cnt_II*3+RADIO_DATA_OFFSET] |= (uint8_t)((tempVal>>(4*(Cnt_III+1)))&Mask3[Cnt_III]);
  Radio_Buffer[Cnt_III+Cnt_II*3+RADIO_DATA_OFFSET+1] |=(uint8_t) ((tempVal<<(4-4*Cnt_III))&Mask4[Cnt_III]);
  Cnt_III++;
  if (Cnt_III == 2)
  {
    Cnt_III=0;
...
/////////////VG 12 
  ...
  if (ADC_Counter == PACKET_PAYLOAD_LENGTH) //VG this makes sense. Ensure packet payload length changes properly
  {
    ...
    //VG12, added if statements to check for ADC resolution ==12
    if ((ADC_Counter == SMALL_PACKET_PAYLOAD) || (ADC_Counter == SMALL_PACKET_PAYLOAD_10Bit) || (ADC_Counter == SMALL_PACKET_PAYLOAD_12Bit)){
      Radio_Buffer[4] = ADC_Resolution == 8 ? (uint8_t)Data_8bit_Small : (uint8_t)Data_10bit_Small;//VGVG verified
      if (ADC_Resolution == 12){
        Radio_Buffer[4] = (uint8_t)Data_12bit_Small;
      }
    } else {
      Radio_Buffer[4] = ADC_Resolution == 8 ? (uint8_t)Data_8bit_Large : (uint8_t)Data_10bit_Large; //VGVG verified
      if (ADC_Resolution == 12){
        Radio_Buffer[4] = (uint8_t)Data_12bit_Large;
      }
    }
...
void TIMER0_IRQHandler(void)
{
  ...
  else
  {
    ...
    {
      SetStimGlitchSwitch(false);
      // //tx_data[0] = positiveStimCode>>8; //Old DAC
